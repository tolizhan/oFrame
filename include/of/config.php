...
return array(
    ...
    'session'     => array(
        ...
        //禁止 js 读取 session_id
        'httpOnly'    => true,
        //正则匹配"调度类名::方法名"判断是否自动开启, 默认""=不自动开启, 以"@"开头的字符串=正则匹配
        'autoStart'   => '@^(?!(serv|of)(_|\b))@',
        //最大生存时间(分钟)
        'maxLifeTime' => 60,
        //各调度参数
        'params'      => array(
    ...
    'com'         => array(
        ...
        'com' => array(
            ...
            'paging' => array(
                //默认展示数量
                'size'  => 10,
                //检查是否有权限调用分页, @开头的字符串=正则验证, 否则=遵循回调规则(接收"类名::方法"参数, 返回true=通过)
                'check' => '@paging$@i'
            )
        ),
        //数据操作
        ...
        'kv' => array(
            'default' => array(
                ...
                'params'  => array(
                    ...
                    //是否长连接
                    'persistent' => false,
                    //自定义分区, 符合框架回调结构({"name" : 操作键名, "host" : 主机列表}), 返回0~n的主机数字
                    'distributor' => null
                    // */
                )
            )
        ...
        'mq' => array(
             ...
             *          "mqSlot" :o生产与消费分槽, 默认自动分槽, 两项一维数组, 如[生产分槽[0, 1], 消费分槽[0, 1, 2]]
             *                     其作用是将消息均衡分布到各节点, 每项对应一个分槽
             *                     选择合适的数字或字符串让每个节点有一个分槽就是最优的方式
             *                     第0项数组代表生产分槽, 总个数代表总分槽数
             *                          自动分槽(推荐), 设置为空数组 且 kvPool类型为分布式模式时生效, 如: []
             *                          手动分槽, 通过手册中"计算分槽数字"方法生成一维数组, 如: [1, 2]
             *                     第1项数组代表消费分槽, 要覆盖生产分槽所有项, 仅手动分槽有效, 自动分槽不用设置
             *                          当生产分槽调整时, 要包含调整前所有项, 待分槽自动调整完后, 改为生产分槽
             *                          注意: 以下情况必须使用"moveMq"进行迁移, 否则会丢消息
             *                              1. kvPool节点数量或顺序变化时
             *                              2. 手动分槽下, kvPool域名变动时
             *                              3. 手动分槽与自动分槽方式切换时
             *      },
             ...
             *              "keys"   : 消费消息时回调结构 {
             *                  消息键 : 不存在的键将被抛弃 {
             *                      "logs" : 跟踪日志, 在控制台中可以查看, >0开启日志, 1=自动日志, 2=回溯日志, 3=1|2日志
             *                          调用 of_base_com_timer::log() 可自定义跟踪日志
