    ...
    if(content === '')
    {
        ...
        switch (contentType) {
            case "url":
                ...
                {
                    content_array[2] = content.slice(content_array[1].length + 6);
            ...
            case "iframe":
                ...
                {
                    var content_array = [content.slice(0, content.indexOf('?')), content.slice(content.indexOf('?') + 1)];
                ...
                if(postIframeForm !== '')    //需要post方式请求iframe
                {
                    $(postIframeForm).insertAfter("#oDialogDiv_iframe_" +dateTime, callBackObj.oDialogDivObj)
                                     .submit()
                                     .remove();
                } else if(L.browser.msie === L.browser.mozilla && content === 'about:blank') {
...
if(typeof oDialogDiv.getTreeNode !== 'function'){
    ...
    **/
    oDialogDiv.skinLayout = function(layoutObj, width, height, topNum, effect, layoutFun)
    {
        ...
        {
            var IE6SelectMaskIframeObj = L.browser.msie && L.browser.version == 6 ? layoutObj.children('iframe').hide() : false;    //解决 IE6 下SELECT元素不能被遮住的bug:调整高度之前隐藏遮罩层,防止影响动画效果
            ...
            var animateAfterFun = function()    //动画结束后调用函数
            {
                ...
                if(height === 'auto' && L.browser.msie && L.browser.version < 8 && width !== 'auto' && oDialogDivContentObj.outerWidth(true) > oDialogDivScrollObj.prop('clientWidth'))
                {
                    ...
                    if(documentObj.width() >= iframeWindowObj.width())    //当resize事件布局结束时
                    {
                        if(arguments.callee.scrollWidth)    //设置iframe的宽
                        {
                            if(
                                ...
                                if(L.browser.msie && L.browser.version < 7)
                                {
    ...
    oDialogDiv.tip = function(text, timeout, nodeNum, mask)
    {
        ...
        if(typeof text === 'undefined')    //移除提示
        {
            ...
            if(
                ...
                if(L.browser.msie && L.browser.version == "6.0" || document.documentMode === 5)
                {
