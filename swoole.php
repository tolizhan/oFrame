 ...
 *          变量隔离 : __construct __destruct __set offsetUnset offsetExists offsetSet offsetGet shareVar varMaps
 *          协程隔离 : dispatch loadClass checkExit onEcho loadEnv reinit clear fork errCtrl autoClass
 *          服务支撑 : data task health time serial yield sentry getBitSize request webSocket server start
 *          代码重写 : codeParser codeKey codeExp codeHard

 *      内部保留变量
 *          _ENV_mapVar : 映射 $_ENV 变量
 *          _SERVER_mapVar : 映射 $_SERVER 变量
 *          _GET_mapVar : 映射 $_GET 变量
 *          _POST_mapVar : 映射 $_POST 变量
 *          _COOKIE_mapVar : 映射 $_COOKIE 变量
 *          _REQUEST_mapVar : 映射 $_REQUEST 变量
 *          _FILES_mapVar : 映射 $_FILES 变量
 *          _SESSION_mapVar : 映射 $_SESSION 变量
 *          GLOBALS_mapVar : 映射 $_GLOBALS 变量
 *          _FUNC_mapVar : 协程空间内部函数调用对象, 用来接管方法 或 操作对应空间功能
 *          _TEMP_mapVar : 临时变量数组, 用来存储临时变量
 *          _GLOBAL_SCOPE_ : 全局空间标记, 若脚本位置在$GLOBALS位置, 则该值存在
 *          _CLOSURE_SCOPE_ : 闭包静态变量空间区分, 在闭包静态变量位置插入该变量, 每个闭包该值存不同对象

 *      内部对象类型
 *          协议封装对象 : 通过stream_wrapper_register注册的对象, 用来改写编译的代码
 *          类静态变量对象 : 存在每个类的静态变量中, 起到不同协程见的隔离作用
 *          闭包静态变量对象 : 存在_CLOSURE_SCOPE_变量中, 当闭包销毁时联动析构清理隔离变量
 *          接管方法对象 : 存在_FUNC_mapVar变量中, 用来接管方法 或 操作对应空间功能
 *          Socket对象 : 存储webSocket实现所需的相关信息, 如连接描述符或回调方法
 ...
 *      服务基础信息($bases)结构 : loadEnv 方法初始化 {
 *          "cInfo" : 类信息列表, {
 *              空间类名 : {
 *                  "cIndex" : 类名索引, strtr(空间类名, '\\', '_'),
 *                  "parent" : 父类名, false = 无父类名
 *                  "pIndex" : 父类名索引, false = 无父类名
 *              },
 *              ...
 *          }
 *          "inits" : 初始化回调母版 {
 *              strtr(空间类名, '\\', '_') : {
 *                  回调方法(class::init) => 回调参数 [],
 *                  ...
 *              },
 *              ...
 *          }
 ...
 *                  "init"    : 初始化回调列表, 同self::$bases['inits']结构
 ...
 *                  ...
 *              ]
 *              "aInit" : 异步初始化任务, 在self::$incNo为0时执行 {
 *                  类名索引 : 类名索引,
 *                  ...
 *              }
 *              "error" : 错误处理数据 {
...
class swoole implements ArrayAccess {
    ...
    //服务基础信息
    private static $bases = array('cInfo' => array(), 'inits' => array(), 'sData' => array());
    //协程隔离列表
    ...
    //代码调试, [(1=重写代码; 2=解析信息), 匹配过滤]
    private static $debug = array(0, '');
    ...
    public function stream_open($path, $mode, $options, &$file) {
        ...
        //执行字符串代码
            ...
            //生成eval路径, 是框架执行协议 ? 截取路径 : 生成路径
            $file = substr($path[2], 0, 8) === 'of.eval:' ? substr($path[2], 10) : "{$path[2]} : eval()'d code";
            //存储改写代码
            $code = '<?php ' . $path[3];
            //缓存标记
            $mark = md5($path[3]);
    ...
    /**
     * 描述 : 注意析构时self::$attrs[space]可能已不存在
     * 作者 : Edgar.lee
     */
    public function __destruct() {
        ...
        if ($this->cMark) {
            //闭包销毁时联动清理隔离变量
            unset(self::$attrs[$this->space]['share'][$this->cMark]);
        }
    }
    ...
    public function &shareVar($list, $site = '') {
        ...
        //静态变量
            ...
            //匿名方法中的静态变量
            if (isset($list['_CLOSURE_SCOPE_'])) {
                //未初始化闭包标识
                if ($list['_CLOSURE_SCOPE_'] === 0) {
                    $list['_CLOSURE_SCOPE_'] = new self;
                    $list['_CLOSURE_SCOPE_']->space = $this->space;
                    $list['_CLOSURE_SCOPE_']->cMark = $site . ' #' . ++$uuid;
                }
                //确定变量位置
                $site = $list['_CLOSURE_SCOPE_']->cMark;
                //不作为静态变量, 仅作为闭包标识
                unset($list['_CLOSURE_SCOPE_']);
            }
            //引用隔离位置的变量
    ...
    public function header($info, $replace = true, $code = 0) {
        ...
        //没有头信息, 输出响应体时会发送头信息并清除头信息列表
        } else if (!$heads) {
            //读取代码行数
            $temp = $this->debugBacktrace(DEBUG_BACKTRACE_IGNORE_ARGS);
            //生成错误
            self::errCtrl(
                E_WARNING,
                "Cannot modify header information - headers already sent by (output started at {$this->print})",
                $temp[0]['file'] ?? __FILE__,
                $temp[0]['line'] ?? __LINE__
            );
        //发送状态
    ...
    public function setTimeLimit($value) {
        self::$attrs[$this->space]['state']['exeTime'] = array($value, self::time() + $value);
        return true;
    }
    ...
    public function date($format, $time = null) {
        return (new DateTime($time === null ? 'now' : '@' . ($time ? $time : 0)))
    ...
    public function strtotime($date, $time = null) {
        try {
            //时间转换
            if ($time === null) {
                return (new DateTime($date, self::$attrs[$this->space]['state']['tzIds'][2]))
                    ->getTimestamp();
            //时间计算
            } else {
                //类型转换
                if (is_bool($time) || is_numeric($time)) {
                    //时间转换
                    $time = self::errCtrl()->errOff = (new DateTime('@' . (int)$time))
                        ->setTimezone(self::$attrs[$this->space]['state']['tzIds'][2])
                        ->modify($date);
                ...
                //无效数据
                    ...
                    self::errCtrl(
                        E_WARNING,
                        'strtotime() expects parameter 2 to be integer, ' . gettype($time) . ' given',
                        $temp[0]['file'] ?? __FILE__,
                        $temp[0]['line'] ?? __LINE__
                    );
                    return false;
    ...
    public function iniSet($option, $value) {
        switch ($option) {
            ...
            case 'max_execution_time':
                ...
                $result = $index[0];
                //设置超时秒数
                $index = array($value, self::time() + $value);
                //返回原数据
                return $result;
            //禁止内存调整, 会影响全局
            case 'memory_limit':
    ...
    public function define($name, $value) {
        //空间常量const, 改写后若__NAMESPACE__为"", 会导致\CONST无效数据情况
    ...
    /**
     * 描述 : 接管 get_defined_vars
     * 作者 : Edgar.lee
     */
    public function getDefinedVars($vars) {
        //过滤内部变量列表
        unset(
            $vars['_ENV_mapVar'], $vars['_SERVER_mapVar'], $vars['_GET_mapVar'],
            $vars['_POST_mapVar'], $vars['_COOKIE_mapVar'], $vars['_REQUEST_mapVar'],
            $vars['_FILES_mapVar'], $vars['_SESSION_mapVar'], $vars['GLOBALS_mapVar'],
            $vars['_FUNC_mapVar'], $vars['_GLOBAL_SCOPE_'], $vars['_CLOSURE_SCOPE_'],
            $vars['_TEMP_mapVar']
        );
        return $vars;
    }
     ...
     * 作者 : Edgar.lee
     */
    public function streamWrapperRegister($name, $class, $flag = 0) {
        static $list = array();
        $list += array($name => -1);
        ++$list[$name] ? $list[$name] = 1 : stream_wrapper_register($name, $class, $flag);
        return true;
    }
    ...
    public function loadClass($class, $isNew = false) {
        if (is_string($class)) {
            ...
            //存在待初始化列表
            if ($init = &self::$attrs[$this->space]['state']['init']) {
                //引用并初始化类信息
                ($cInfo = &self::$bases['cInfo'][$class]) === null && $cInfo = array(
                    'cIndex' => strtr($class, '\\', '_'),
                    'parent' => $temp = get_parent_class($class),
                    'pIndex' => $temp ? strtr($temp, '\\', '_') : false
                );

                //存在父类 && 父类存在初始化回调 && 递归初始化父类
                $cInfo['pIndex'] && isset($init[$cInfo['pIndex']]) && self::loadClass($cInfo['parent']);

                //存在类初始化回调
                isset($init[$cInfo['cIndex']]) && self::reinit($cInfo['cIndex']);
            }
        }
     ...
     * 作者 : Edgar.lee
     */
    private function checkExit($mode = 7) {
        ...
        //协程超时退出
            ...
            self::errCtrl(
                E_ERROR,
                "Maximum execution time of {$exeTime[0]} second exceeded",
                $temp[0]['file'] ?? __FILE__,
                $temp[0]['line'] ?? __LINE__
            );
            exit;
            ...
            if ($memory[1] > -1 && (!$exeTime[0] || $GLOBALS['system']['open'] === 'task')) {
                ...
                if (isset($reList[$this->space])) {
                    ...
                    if ($memory[3] > $memory[1]) {
                        ...
                        self::errCtrl(
                            E_ERROR,
                            "Failed to set memory limit to {$memory[1]} bytes (Current memory usage is {$memory[3]} bytes)",
                            $temp[0]['file'] ?? __FILE__,
                            $temp[0]['line'] ?? __LINE__
                        );
                        exit;
     ...
     * 作者 : Edgar.lee
     */
    private function onEcho($buffer) {
        //有响应头
        if ($heads = &self::$attrs[$this->space]['heads']) {
            //发送响应头
            $this->header($this->swRes);
            //标记响应头已发送
            $heads = array();

            //读取代码行数
            $temp = $this->debugBacktrace(DEBUG_BACKTRACE_IGNORE_ARGS)[0] + array('file' => '', 'line' => '');
            //记录首次输出的位置
            $this->print = "{$temp['file']}:{$temp['line']}";
        }

        //有响应对象 && 非功能操作 && 发送响应体
        $this->swRes && isset($buffer[0]) && $this->swRes->write($buffer);
        //有响应对象 && 非功能操作 && 不设置buffer_output_size的替代或结合方案(分块影响gzip压缩比)
        //$this->swRes && isset($buffer[0]) && array_map(array($this->swRes, 'write'), str_split($buffer, 1048576));
        //返回空字符串
        return '';
    }
    ...
    public static function &loadEnv(&$requ = null, &$resp = null) {
        ...
        if ($requ) {
            ...
            //捕获所有输出
            ob_start(array($func = new self, 'onEcho'), 1);
            //设置协程ID
            $func->space = $space;
     ...
     * 作者 : Edgar.lee
     */
    public static function reinit($call = 'of', $type = true) {
        //协程ID
        $space = Co::getCid();
        ...
        if ($type) {
            //引用协程初始化回调列表
            $init = &self::$attrs[$space]['state']['init'];
            //存在类初始化回调
            if (isset($init[$call])) {
                //引用类初始化回调列表
                $index = &$init[$call];
                ...
                unset($init[$call]);
                //完成类初始化回调
                foreach ($index as $k => &$v) call_user_func_array($k, $v);
        ...
        //首次注册, 在所有已初始的的协程中执行
            ...
            //类名索引
            $cIndex = strtr(strstr($call, '::', true), '\\', '_');

            //禁用协程
            self::serial(true, $space);
            //未被并行的协程初始, 初始化其它已初始化过的协程
            if (($index = &self::$bases['inits'][$cIndex][$call]) === null) {
                //记录回调母版
                $index = array();
                ...
                foreach (self::$attrs as $k => &$v) {
                    //追加初始回调
                    $v['state']['init'][$cIndex][$call] = array();
                }
            }
            //异步初始化任务
            self::$attrs[$space]['aInit'][$cIndex] = $cIndex;
            //启用协程
            self::serial(false, $space);
        }
    }
    ...
    public static function clear($type = null) {
        ...
        //协程资源释放
            ...
            if ($index->swRes) {
                ...
                //响应输出信息, 当调用ob_get_clean方法时, 触发的onEcho方法会发送响应头
            ...
            if ($eMark) {
                foreach ($eData['catch'] as $k => &$v) {
                    ...
                    if ($v['mark'] === $eMark) {
                        ...
                        self::$incNo === $v['iNum'] || self::serial($v['iNum'], $space);
                        //恢复完成
                        break ;
                    }
                }
     ...
     * 参数 :
     *      name : 数据名称
     *      data : 初始数据, 默认[]=创建(data首次默认值, time每次都生效), true=读取, false=删除
     *      time : 过期时间, data为数组时生效, 默认86400=一天有效, 0=永不过期
     * 返回 :
     *      数据存在 ? &返回数据 : 返回null
     * 作者 : Edgar.lee
     */
    public static function &data($name, $data = array(), $time = 86400) {
        //引用共享数据
        $list = &self::$bases['sData'];

        //移除数据, 删除 || 数据不存在 || (设置过期 && 已经过期)
        if ($data === false || (!$index = &$list[$name]) || $index[1] && $index[1] < self::$nTime) {
            unset($list[$name], $index);
        }

        //初始化数据
        if (is_array($data)) {
            $list += array($name => array(&$data, 0));
            $index = &$list[$name];
            $index[1] = $time ? self::time() + $time : 0;
        }
    ...
    private static function autoClass($class, $serial = true) {
        ...
        $serial && self::serial(true, $space);
        //引用自动加载类
        foreach (self::$attrs[$space]['autoC'] as &$v) {
            //触发加载回调
        ...
        //关闭串行化
        $serial && self::serial(false, $space);
    }
    ...
    private static function time() {
        //更新最近时间
        return self::$nTime = time();
    }

    /**
     * 描述 : 开关串行化
     * 参数 :
     *      sMode : 串行模式, true=开启, false=禁用, int=恢复到指定层级
     *      space : 协程ID, 用于指定协程的串行化
     * 返回 :
     *      返回操作前串行层次
     * 作者 : Edgar.lee
     */
    private static function serial($sMode, $space) {
        //引用串行化层级
        $incNo = &self::$incNo;
        //返回操作前串行层次
        $result = $incNo;
        ...
        if ($sMode) {
            ...
            //非数字 ? 增加包含层次 : 设置指定值
            $sMode === true ? ++$incNo : $incNo = $sMode;
        //恢复协程
        } else {
            //是数字 && 串行状态 && 准备关闭串行
            $sMode === 0 && $incNo && $incNo = 1;

            //串行状态 && 可以关闭串行
            if ($incNo && --$incNo === 0) {
                //开启函数协程化
                Swoole\Runtime::enableCoroutine(SWOOLE_HOOK_FULL);
                //开启抢占式调度
                SWOOLE_SCHEDULER && Co::enableScheduler();
            }

            //异步初始化任务
            if ($incNo === 0 && $aInit = self::$attrs[$space]['aInit']) {
                //重置初始化队列
                self::$attrs[$space]['aInit'] = array();
                //异步初始化任务
                foreach ($aInit as &$v) self::reinit($v);
            }
    ...
    private static function yield($type, $data = array()) {
        ...
        //标记有待办任务且守望者停止 && 恢复守望者
        $incNo = (self::$yield['state']['code'] |= 2) === 2 ? self::serial(0, $space) : 0;
        //挂起协程
        Co::yield();
        //恢复串行状态
        $incNo && self::serial($incNo, $space);
    ...
    private static function sentry($serv, $mCid) {
        ...
        go(function () use (&$serv, &$mCid) {
            //引用共享数据
            $sData = &self::$bases['sData'];
            //引用进程最大内存
            $memory = &$GLOBALS['system']['memory'];
            //引用工作周期时间
            ($cycle = &$GLOBALS['system']['cycle']) && $cycle += self::time();
            ...
            do {
                ...
                foreach ($sData as $k => &$v) {
                    //设置过期 && 已经过期
                    if ($v[1] && $v[1] < self::$nTime) unset($sData[$k]);
                }
            } while (!self::$isEnd);
     ...
     *          "isOn" : 是否需要缓存, 0=不需要, 1=需要, 仅在全局类型下可能生效
    ...
    private static function codeParser(&$code, $gVar, &$ctrl = false) {
        ...
        //类属性修饰符
        static $aMode = array(T_PUBLIC => 1, T_PROTECTED => 1, T_PRIVATE => 1, T_STATIC => 1, ')' => 1);
        //类的修饰符
        ...
        static $func = array(
            ...
            'call_user_func' => 'callable', 'get_defined_vars' => 'getDefinedVars'
        );
        //唯一ID计数
        static $uuid = 0;

        //调试代码
        $test = self::$debug[0] && (!self::$debug[1] || strpos($code, self::$debug[1]) !== false);
        //待处理列表
        $wait = array();
        //代码树列表, 记录类和方法的顺序
        ...
        foreach ($keys as $kk => &$vk) {
            ...
            if (isset($vk[2])) {
                ...
                switch ($vk[0]) {
                            ...
                            for ($i = $kk; ++$i;) {
                                if ($keys[$i][0] === ';') {
                                    ...
                                    $keys[$i][0] === '=' && $wait[] = array(
                                        'pos' => $kPos,
                                        'len' => $kLen + $temp,
                                        'str' => '$_FUNC_mapVar->define(__NAMESPACE__ . \'\\' . self::codeKey($keys, $kk, 1) . '\', '
                                    );
                                    $temp += isset($keys[$i][2]) ? strlen($keys[$i][1]) : 1;
                                }
                            }
                    ...
                    //箭头函数
                    case T_FN:
                        //退出静态变量, 可能出现静态箭头函数 static fn()
                        $self['isStatic'] = 0;
                        //硬缓存代码未开启 && 在全局 && 开启硬缓存代码
                        !$hard['isOn'] && $self['type'] === 1 && $hard['isOn'] = 1;
                        break;
                    //方法
                    case T_FUNCTION:
                        //退出静态变量, 可能出现静态匿名方法 static function()
                        $self['isStatic'] = 0;
                    ...
                    case T_STATIC:
                        //是静态操作 || 进入静态变量
                        self::codeKey($keys, $kk, 1) === '::' || $self['isStatic'] = 1;
                        break;
                    //变量
                    case T_VARIABLE:
                        //前后非空关键词
                        ...
                        //独立变量代码, 前面不为::
                            ...
                            //在静态变量中 || 在全局变量中
                            } else if ($self['isStatic'] || $self['isGlobal']) {
                                //记录全局或静态变量
                                $self['varList'][$vk[1]] = '\'' . substr($vk[1], 1) . "' => &{$vk[1]}";

                                //在类或复用中 && 在静态变量中
                                if ($self['type'] & 20 && $self['isStatic']) {
                                    $data = array('ePos' => $kk);
                                    $temp = $kPos;
                                    ...
                                    while (true) {
                                        //类属性有效修饰符
                                        if (isset($aMode[self::codeKey($keys, $data['ePos'], -1, $data, 1)])) {
                                            //存在类属性声明 && 删除 static ?int $var 结构的 ?int 类型声明
                                            $kPos > $temp && $wait[] = array(
                                                'pos' => $temp,
                                                'len' => $kPos - $temp,
                                                'str' => ''
                                            );
                    ...
                    case T_STRING:
                        if (
                            //在类(4)或复用(16)中 || 在方法的"("中
                            ($self['type'] & 20 || ($self['type'] & 2 && $self['life'] < 0)) &&
                            //定位到属性钩子
                            ($vk[1] === 'set' || $vk[1] === 'get')
                        ) {
                            for ($i = $kk; ++$i;) {
                                switch ($keys[$i][0]) {
                                    //寻找"{", 属性钩子是个方法体, 压入未初始化的匿名方法
                                    case '{':
                                        array_unshift($tree, array('type' => 2, 'name' => '', 'uuid' => ++$uuid) + $strur);
                                        break 2;
                                    //寻找";"或"=>"位置, 结束属性钩子方法体的寻找
                                    case ';':
                                    case T_DOUBLE_ARROW:
                                        break 2;
                                }
                    ...
                    case T_NAME_FULLY_QUALIFIED:
                        if (
                            (
                                ...
                                //命中接管名称
                                isset($func[$lf = strtolower(ltrim(self::codeExp($keys, $kk + 1, -1), '\\'))])
                            ) && (
                                //独立语句
                            ...
                            //接管get_defined_vars
                            } else if ($func[$lf] === 'getDefinedVars') {
                                $wait[] = array(
                                    //在"("之前调用更安全
                                    'pos' => $kPos,
                                    'len' => $kLen + $data['eLen'],
                                    'str' => "\$_FUNC_mapVar->getDefinedVars(get_defined_vars()"
                                );
                            //接管指定方法, \方法名
                            } else if ($temp['text'] === '\\') {
                                $wait[] = array(
                        ...
                        if (
                            ...
                            if (
                                ...
                                $wait[] = array(
                                    ...
                                    //获取命名含空间的类名::init
                                    'str' => "\swoole::reinit({$data['name']} . '::{$temp['text']}', false"
                                );
            ...
            //是字符
                ...
                switch ($vk) {
                    ...
                    case '(':
                        //在方法中 && 未进入方法周期 && 周期+1
                        $self['type'] === 2 && $self['life'] <= 0 && --$self['life'];
                        break;
                    case ')':
                        //在方法中 && 未进入方法周期 && 周期-1
                        $self['type'] === 2 && $self['life'] < 0 && ++$self['life'];
                    ...
                    case '{':
                        ...
                        $ctrl = true;
                        //记录生命周期
                        $self['isQuotes'] || $self['life'] >= 0 && ++$self['life'];
                        break;
                    case '}':
                        //销毁代码树
                        if ($self['isQuotes'] === 0 && $self['life'] >= 0 && --$self['life'] === 0) {
                            //类的结束
                            if ($self['type'] === 4) {
                    ...
                    case ';':
                        ...
                        //在全局或方法中 && 有共享变量
                        if ($self['type'] & 3 && $self['varList']) {
                            //在方法中 && 匿名函数
                            if ($self['type'] & 2 && !$self['name']) {
                                //改写静态或全局的变量列表, 区分每个不同的匿名方法
                                $wait[] = array(
                                    'pos' => $kPos,
                                    'len' => 0,
                                    'str' => ', $_CLOSURE_SCOPE_ = 0'
                                );
                                //静态变量中增加 $_CLOSURE_SCOPE_
                                $self['varList']['_CLOSURE_SCOPE_'] = '\'_CLOSURE_SCOPE_\' => &$_CLOSURE_SCOPE_';
                            }
                            //改写静态或全局的变量列表
                            $wait[] = array(
                                'pos' => $kPos + 1,
     ...
     *      }
     * 返回 :
     *      text=结果有效, false=关键词类名
     * 作者 : Edgar.lee
     */
    private static function codeExp(&$keys, $kPos, $move, &$data = null) {
        //超全局变量列表
        static $super = array(
            '$GLOBALS' => 1, '$_SERVER' => 1, '$_ENV' => 1,
            '$_GET' => 1, '$_POST' => 1, '$_FILES' => 1,
        ...
        while (isset($keys[$kPos += $move])) {
            ...
            if ($loop) {
                //记录代码
                $list[] = isset($super[$text]) ? "{$text}_mapVar" : $text;
                //是关键符
                isset($loop[$code]) &&
                    //完成递归闭环
    ...
    private static function codeHard(&$wait, &$hard, $kPos, $pStr = '') {
        ...
        static $uuid = 0;
        //硬缓存随机方法名
        $temp = '_swooleCodeHardCache_' . ++$uuid;
        //插入硬缓存起始代码
        $wait[] = array(
            'pos' => $hard['cPos'],
            'len' => 0,
            'str' => '/*!swoole rewrite: start!*/' .
                '$_FUNC_mapVar->serial = true;' .
                "if (!function_exists(__NAMESPACE__ . '\\{$temp}')) {" .
                'function ' . $temp . '(&$_TEMP_mapVar) {' .
                    //方法外部变量引用到方法内部
                    'extract($_TEMP_mapVar, EXTR_REFS);' .
                    'try {'
        );
        //插入硬缓存结束代码
        $wait[] = array(
            ...
            'str' => $pStr .
                    '} finally {' .
                        //方法内部变量引用到方法外部
                        '$_TEMP_mapVar = array($_FUNC_mapVar->getDefinedVars(get_defined_vars()));' .
                        'foreach ($_TEMP_mapVar[0] as $_TEMP_mapVar[1] => &$_TEMP_mapVar[2]) $_TEMP_mapVar[0][$_TEMP_mapVar[1]] = &${$_TEMP_mapVar[1]};' .
                        '$_TEMP_mapVar = $_TEMP_mapVar[0];' .
                    '}' .
                '}}$_FUNC_mapVar->serial = false;/*!swoole rewrite: end!*/' .
                //准备引用到方法内部到变量
                '$_TEMP_mapVar = array(get_defined_vars());' .
                'unset($_TEMP_mapVar[0][\'_TEMP_mapVar\']);' .
                'foreach ($_TEMP_mapVar[0] as $_TEMP_mapVar[1] => &$_TEMP_mapVar[2]) $_TEMP_mapVar[0][$_TEMP_mapVar[1]] = &${$_TEMP_mapVar[1]};' .
                //调用方法传递引用变量
                '$_TEMP_mapVar[1] = ' . $temp . '($_TEMP_mapVar[0]);' .
                //方法内部变量引用到方法外部
                'extract($_TEMP_mapVar[0], EXTR_REFS);' .
                'if ($_TEMP_mapVar[1] !== null) {return $_TEMP_mapVar[1];}'
        );
        //重置硬缓存
    ...
    private static function &request($requ, $resp) {
        ...
        if ($GLOBALS['system']['routes']) {
            ...
            foreach ($GLOBALS['system']['routes'] as $k => &$v) {
                //按路由规则替换
                $file = preg_replace($k, $v, $file, -1, $temp);
                //路由成功
                if ($temp) {
                    //路由改写出GET参数
                    if ($temp = parse_url($file, PHP_URL_QUERY)) {
                        //同时存在请求参数 && 合并GET参数
                        empty($index['query_string']) || $temp .= '&' . $index['query_string'];
                        //生成新的GET参数数组
                        parse_str($temp, $requ->get);
                        //编码GET参数为字符串(去掉多余干扰字符)
                        $index['query_string'] = http_build_query($requ->get);
                    }
    ...
    public static function start() {
        ...
        $config = array(
            ...
            'server' => array(
                ...
                'heartbeat_check_interval' => 43200, 'buffer_output_size' => PHP_INT_MAX
            ),
            'phpIni' => array(
